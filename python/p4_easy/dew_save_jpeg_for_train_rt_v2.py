...
lastmove_mili = time.time()
#lastmove_mili = int(round(time.time()) * 1000)
cursave = 0
...
while not done:
    ...
    if(tmp_mili <= preparing_mili):
        ...
        label = myfont.render("please preparing your self... %f " %((preparing_mili- tmp_mili)), 1, (0,255,255))
        screen.blit(label, (100, 100))

    elif (tmp_mili > preparing_mili):
        myfont = pygame.font.SysFont("monospace", 40)
        label = myfont.render("Capturing your motion... %f secords left" %((capture_mili- tmp_mili)), 1, (255,0,255))
        screen.blit(label, (100, 100))
        ...
        lasmove_mili = cur_mili


    adx =  ((cur_mili - lastmove_mili) ) * 0.5
    ady =  ((cur_mili - lastmove_mili) ) * 0.5
#clock.tick(60)
  #  adx =  ((cur_mili - lastmove_mili) ) * 300
  #  ady =  ((cur_mili - lastmove_mili) ) * 300

    adx = int(adx)
    ady = int(ady)

    if (curx >= screen_width - 3):
        asignx = asignx * -1

    if (cury >= screen_height - 3):
        asigny = asigny * -1
