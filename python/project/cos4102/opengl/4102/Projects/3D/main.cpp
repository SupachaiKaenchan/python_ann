...
GLfloat theta = 0.0;
GLint sign = 1;
//GLfloat x = 0.0,y = 12.0,z = 0.0;///??????????x
GLfloat x = 0.0,y = 0.0,z = 14.0;///??????????y
//GLfloat x = 11.0,y = 0.0,z = 0.0;///??????????z
GLfloat angle = 0.0;
void Init(void);
void Transform(void);
...
//       SetLookAt(((cos(angle)*x)+(sin(angle)*z)),y,((-sin(angle)*x)+(cos(angle)*z)),0,0,0,0,1,0);
//     SetLookAt(((cos(angle)*x)+(sin(angle)*y)),((-sin(angle)*x)+(cos(angle)*y)),z,0,0,0,0,0,1);
...
void PlotSurface(Point Surface[RN],GLfloat red,GLfloat green,GLfloat blue)
{
    ...
    glBegin(GL_POLYGON);
        for (int i=0; i<RN; i++)
            glVertex4f(Surface[i].x, Surface[i].y, Surface[i].z, 0.75);
    glEnd();
}
